...
import {
  type AdvisorInfoData,
  type AdvisorSource,
  DEFAULT_ADVISOR_INFO_DATA,
} from './AdvisorInfoConfig';

/* ────────────────────────── props ────────────────────────── */
interface AdvisorInfoPageProps {
  data?: AdvisorInfoData;
  onDataChange?: (d: AdvisorInfoData) => void;
}
...
export default function AdvisorInfoPage({
  data: externalData,
  onDataChange,
}: AdvisorInfoPageProps) {
  const { isPresentation } = useViewMode();
  const [data, setData] = useState<AdvisorInfoData>(
    externalData ?? DEFAULT_ADVISOR_INFO_DATA
  );
  ...
  const update = useCallback(
    <K extends keyof AdvisorInfoData>(key: K, value: AdvisorInfoData[K]) => {
      setData((prev) => {
  ...
  /* ── source helpers ── */
  const addSource = () =>
    update('sources', [...data.sources, { label: '', url: '' }]);
  const removeSource = (i: number) =>
    update(
      'sources',
      data.sources.filter((_, idx) => idx !== i)
    );
  const updateSource = (i: number, field: keyof AdvisorSource, v: string) => {
    const next = data.sources.map((s, idx) =>
      idx === i ? { ...s, [field]: v } : s
    );
    update('sources', next);
  };
  ...
  const removeWorkItem = (i: number) =>
    update(
      'workStyle',
      data.workStyle.filter((_, idx) => idx !== i)
    );
        ...
        <div className="max-w-2xl mx-auto px-6 py-16 md:py-24 space-y-16">
          ...
          <PresentationSection delay={0}>
            ...
            <p className="text-lg text-foreground/80 leading-relaxed max-w-prose">
              Mir ist wichtig, dass du genau verstehst, wie ich arbeite – und
              dass du alles jederzeit überprüfen kannst.
            </p>
          </PresentationSection>
          ...
          <PresentationSection delay={100}>
            ...
            <p className="text-[15px] text-muted-foreground leading-relaxed mb-6">
              Ich bin offiziell registriert und zertifiziert. Alle Angaben sind
      ...
      <div className="p-6 md:p-8 max-w-3xl mx-auto space-y-8">
        ...
        <div className="space-y-1">
          ...
          <p className="text-sm text-muted-foreground">
            Verwalte Registrierungslinks, Vergütungsinfos und Quellen für die
            Kundenpräsentation.
          </p>
        </div>
        ...
        <div className="flex items-center gap-3 p-4 rounded-lg border bg-muted/30">
          ...
          <Switch
            checked={data.showInPresentation}
            onCheckedChange={(v) => update('showInPresentation', v)}
          />
        </div>
        ...
        <section className="space-y-4">
          ...
          <Field
            label="Registrierungslink (z. B. FINMA)"
            value={data.registrationLink}
            onChange={(v) => update('registrationLink', v)}
          />
          <Field
            label="Qualifikationslink (z. B. Cicero / IAF)"
            value={data.qualificationLink}
            onChange={(v) => update('qualificationLink', v)}
          />
        </section>
        ...
        <section className="space-y-4">
          ...
          <Field
            label="Beschreibung Vergütung"
            value={data.compensationText}
            onChange={(v) => update('compensationText', v)}
            multiline
            rows={4}
          />
        </section>
          ...
          {data.workStyle.map((item, i) => (
            <div key={i} className="flex gap-2 items-start">
              <Input
                ...
                placeholder="z. B. Analyse statt Verkauf"
              />
              <Button
                variant="ghost"
                size="icon"
